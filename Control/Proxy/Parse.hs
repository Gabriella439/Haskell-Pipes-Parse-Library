...
import Control.Proxy.Trans.State (StateP(StateP, unStateP), put, get, modify)
...
draw
    :: (Monad m, Proxy p)
    => StateP [Maybe a] p () (Maybe a) y' y m (Maybe a)
draw = do
    s <- get
    case s of
        []   -> request ()
...
unDraw
    :: (Monad m, Proxy p)
    => Maybe a -> StateP [Maybe a] p x' x y' y m ()
unDraw ma = modify (ma:)
...
passUpTo
    ...
    -> (() -> StateP ([Maybe a], Sum Int) p () (Maybe a) y' y m (Maybe a))
    -- ^ Read handle
passUpTo n () = do
    Sum m <- zoom _snd get
    if (m < n)
