...
-}
spans
    :: (Monad m, Profunctor p, Functor f)
    => (a -> Bool)
    -- ^
    -> p (Producer a m (Producer a m r)) (f (Producer a m (Producer a m r)))
    -- ^
    -> p (Producer a m               r ) (f (Producer a m               r ))
spans predicate = dimap (splitUsing (takeWhile' predicate)) (fmap join)
...
splitsAt
    ...
    -- ^
    -> p (Producer a m               r ) (f (Producer a m               r ))
splitsAt n = dimap (splitUsing (P.take n)) (fmap join)
...
groupsBy
    ...
    -- ^
    -> p (Producer a m               r ) (f (Producer a m               r ))
groupsBy equal = dimap (splitUsing equals) (fmap join)
...
{-| 'splitUsing' is a utility function that library writers can use to build
    ...
    isomorphism.  See the source code to 'splitsAt', 'spans', and 'groupsBy' for
    example usage.
-}
splitUsing
    :: (Monad m)
